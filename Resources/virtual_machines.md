...
operating system that allows you ro access a virtual computer from any other 
...
I would recommend setting up the Linux machine.  It is simpler to set up and
...
* Select the type of VM you would like.
  ...
  + To select the course-specific Linux option, look for an entry similar to  
    ...
    Note: it may also be under the "My Courses" tab.
...
<https://vcm.duke.edu>.  
* Clicking on the name of your virtual server will allow you to access the VM
...
as your NetID.  You can ignore the Admin username and password.
...
options for SSH clients.  
  + For macOS and Windows, there is a built-in SSH client you can run from the 
    ...
    + Type `ssh user@hostname` where `user` is the user account to be accessed
      ...
      example: `aa123@vcm-11111.vm.duke.edu`.  
...
#### Using MobaXterm (Personal Edition v25.4)
...
* To make future logins easier, you can enter your NetID in the "Specify 
  Username" field.  
* Click "Ok" and a terminal window should open.  If it is your first time
  accessing the VM, you may see a window prompting you to confirm access.
* If you didn't include your NetID in the step above, you will be asked for 
  your NetID in the terminal.
* You will then be asked for your password.  Enter your NetID password followed
  by "Enter".  You will not see any characters in the terminal when typing your 
  password.
...
The following instructions have been developed using a local Windows 11 machine.
...
* To disconnect, simply choose "Disconnect" from the Power button on the Start
  menu.  This will
...
### For the Linux Ubuntu 24.04 VMs, as of December 2025
The default installation has `python3` (version 3.12.3) installed.  And, it 
does not have `pip3` installed or the `venv` package.  So, you will need to 
complete the following steps.
* Install `pip3` by using the following command:
  ```bash
  sudo apt install python3-pip  
  ```
  Enter your Duke NetID password if prompted.
* If you want to avoid needing to enter `python3` and `pip3`, you can 
  ...
  ```
  Note: You will need to exit out of and re-log into the VM for the alias 
  changes to take effect.
* Install the `venv` virtual environment package with the following command:
  ```bash
